...
class LlavaMetaForCausalLM(ABC):
    ...
    def landmarkencoding(self,landmarks):
        mark_num = landmarks.shape[-2]
        # print(landmarks.shape)
        angles = []
        for i in range(mark_num):
            for j in range(i+1,mark_num):
                for k in range(j+1,mark_num):
                    v1 = landmarks[:,:,j] - landmarks[:,:,i]
                    v1[v1==0] = 1e-10
                    v2 = landmarks[:,:,k] - landmarks[:,:,i]
                    v2[v2==0] = 1e-10
                    cos_angle = torch.sum(v1*v2,dim=-1)/(torch.norm(v1,dim=-1)*torch.norm(v2,dim=-1))
                    angles.append(cos_angle)
        landmarks_feature = torch.stack(angles,dim=-1)[:,:,0:1024].unsqueeze(2)
        return landmarks_feature.bfloat16()
    def encode_videos(self, videos,landmarks):  # [mini_b, c, t, h, w]
        b, _, t, _, _ = videos.shape
        video_features = self.get_model().get_video_tower()(videos)  # [mini_b, t, n, c]
        # print(video_features)
        landmark_features = self.landmarkencoding(landmarks)
        video_features = torch.cat([video_features,landmark_features],dim = 2)
        video_features = self.get_model().mm_projector(video_features)
        # print(video_features.shape)
        return video_features

    def prepare_inputs_labels_for_multimodal(
        self, input_ids, position_ids, attention_mask, past_key_values, labels, images,landmarks
    ):
        # ====================================================================================================
        ...
        if getattr(videos_minibatch, 'ndim', 0) == 5:  # batch consists of videos, [mini_b, c, t, h, w]
            video_features_minibatch = self.encode_videos(videos_minibatch,landmarks)  # fake list [mini_b, t, l, c]

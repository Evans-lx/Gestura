...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        try:
            ...
            if 'egogesture' in self.list_data_dict[i]['video']:    
                landmark_file = os.path.join("/gemini-1/space/lat/egogesture_landmarks/", os.path.basename(f"{self.list_data_dict[i]['video']}").replace('.mp4','.npy'))
            else:
                landmark_file = os.path.join("/gemini-1/space/lat/landmarks/", os.path.basename(f"{self.list_data_dict[i]['video']}").replace('.mp4','.npy'))
            landmark = np.load(landmark_file)
            data_dict['landmark'] = landmark
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        '''
        if 'landmark' in instances[0]:
            landmarks = np.array([instance['landmark'] for instance in instances])
            batch['landmarks'] = torch.tensor(landmarks)
